...
function gotResults(error, results){
    if(error){
        ...
        document.getElementById("name_of_sketch").innerHTML = "<h3 id='name_of_sketch'>Your Sketch: " + drawn_sketch + "</h3>";
        percentage = results[0].confidence * 100;
        document.getElementById("confidence").innerHTML = "<h3 id='confidence'>Confidence: " + percentage + "</h3>";
        sketch_to_be_drawn = document.getElementById("sketch_be_drawn");
...
function timerUpdate(){
    document.getElementById("time").innerHTML = 'Timer 00:' + sec;
    const countDown = setInterval(()=>{
        sec--;
        document.getElementById("time").innerHTML = 'Timer 00:' + sec;
    if(sec < 1){
        console.log("next, you failed")
        updateCanvas();
        sec = 6;
    }
    }, 1000)
